...
public class PopSlaveActingMasterIT extends ContainerIntegrationTestBase {
    ...
    public void testLocalActing_notAckSlave() throws Exception {
        ...
        await().atMost(Duration.ofMinutes(5)).pollInterval(Duration.ofSeconds(1)).until(() -> {
            if (retryMsgList.size() < MESSAGE_COUNT) {
                System.out.println("FAILED" +  failCnt.incrementAndGet() + ": retryMsgList.size=" + retryMsgList.size() + " less than " + MESSAGE_COUNT);
                return false;
            }

            for (String msgBodyString : retryMsgList) {
                if (!sendToIsolateMsgSet.contains(msgBodyString)) {
                    System.out.println("FAILED: sendToIsolateMsgSet doesn't contain " + msgBodyString);
                    return false;
                }
            }
            return true;
    ...
    public void testRemoteActing_notAckSlave_getFromLocal() throws Exception {
        ...
        pushConsumer.registerMessageListener((MessageListenerConcurrently) (msgs, context) -> {
            for (MessageExt msg : msgs) {
                System.out.printf("receive retry msg: %s%n", msg);
                retryMsgList.add(new String(msg.getBody()));
            }
            return ConsumeConcurrentlyStatus.CONSUME_SUCCESS;
        });
        ...
        await().atMost(Duration.ofMinutes(1)).until(() -> {
            if (retryMsgList.size() < MESSAGE_COUNT) {
                System.out.println("FAILED: retryMsgList.size=" + retryMsgList.size() + " less than " + MESSAGE_COUNT);
                return false;
            }
